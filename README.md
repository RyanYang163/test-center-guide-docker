...
> 为什么运行时验证在 CI 而不在本机：开发机没有 Docker；实验室设备的 DockerEngine 又不可用
> （实测 2026-09-24：应用能启动但 dockerd 不建 `/var/run/docker.sock`）。runner 自带 Docker 且能访问 Docker Hub，
> 是唯一可靠的运行时验证场所。三个工作流的结果都用 `::notice::` 回传 —— 本机下不了 job 日志（302 跳转那条线断）。
